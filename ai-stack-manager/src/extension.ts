...
} from './infrastructure/services/HealthCheckScheduler';
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const container = new ServiceContainer()
    ...
    .register(
      TOKENS.Installer,
      (c) => new FileInstaller(c.resolve<GitHubMetricsService>(TOKENS.Metrics)),
    )
    .register(
      TOKENS.HealthChecker,
      (c) =>
        new HealthCheckerService(
          c.resolve<GitRegistry>(TOKENS.Registry),
          c.resolve<WorkspaceScanner>(TOKENS.Scanner),
        ),
    )
  ...
  const installedProvider = new InstalledViewProvider(
    context.extensionUri,
    registry,
    scanner,
    installer,
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('dai.uninstall', async () => {
      ...
      for (const pkg of packages) {
        ...
        if (status === InstallStatus.Installed || status === InstallStatus.Partial) {
          installed.push({
            label: `$(trash) ${pkg.displayName}`,
            description: pkg.type.label,
            id: pkg.id,
          });
        }
      }
    ...
    vscode.commands.registerCommand('dai.healthCheck', async () => {
      await healthProvider.refresh();
    }),
    ...
    vscode.commands.registerCommand('dai.installBundle', async () => {
      ...
      const items: Array<{ label: string; description: string; detail: string; id: string }> =
        bundles.map((b: import('./domain/entities/Bundle').Bundle) => ({
          ...
          description: `${b.packageCount} pacotes`,
  ...
  try {
    const handler: vscode.ChatRequestHandler = async (request, _chatContext, stream, _token) => {
      ...
      if (cmd === 'recommend' || (!cmd && prompt.toLowerCase().includes('recommend'))) {
        ...
        if (profiles.length > 0) {
          ...
          if (best.detectedSignals.length > 0) {
            const signals = best.detectedSignals
              .slice(0, 5)
              .map((s) => `\`${s}\``)
              .join(', ');
          ...
          if (recommendedBundle) {
            ...
            for (const pkgId of recommendedBundle.packageIds) {
              ...
              if (pkg) {
                ...
                stream.markdown(
                  `| ${pkg.categoryEmoji || pkg.typeIcon} **${pkg.displayName}** | ${pkg.agentMeta?.category.label ?? pkg.type.label} | ${icon} ${isInst ? 'Instalado' : 'Pendente'} |\n`,
                );
              }
            }
            ...
            if (pending > 0) {
              ...
              stream.markdown(
                `na Command Palette (\`Ctrl+Shift+P\`) e selecione **${recommendedBundle.displayName}**.\n`,
              );
            } else {
        ...
        for (const b of bundles) {
          ...
          stream.markdown(
            `- **${b.displayName}** (${bundleInstalled}/${b.packageCount} instalados) — ${b.description}\n`,
          );
        }
        ...
        if (agents.length > 0) {
          ...
          for (const p of agents) {
            stream.markdown(
              `- ${statusIcon(p.id)} ${p.categoryEmoji} **${p.displayName}** — ${p.description}\n`,
            );
          }
        }
      ...
      if (cmd === 'diff') {
        ...
        const { StackDiffBuilder: Builder } =
          await import('./infrastructure/services/StackDiffBuilder');
        const chosenBundle = target ?? allBundles[0];
        if (chosenBundle) {
          const diff = new Builder().build({
            targetBundle: chosenBundle,
            allPackages: allPkgs2,
            installedIds: installedIds2,
          });
          stream.markdown(`### ${chosenBundle.displayName}\n\n`);
          stream.markdown(
            `**Cobertura:** ${diff.coveragePercent}% (${diff.installed.length}/${diff.installed.length + diff.missing.length} pacotes)\n\n`,
          );

          if (diff.installed.length > 0) {
            stream.markdown('**✅ Instalados:**\n');
            for (const e of diff.installed) {
              stream.markdown(`- ${e.categoryEmoji || '📦'} ${e.displayName}\n`);
            }
            stream.markdown('\n');
          }
          ...
          if (diff.extras.length > 0) {
            stream.markdown(
              `**🔄 Extras (fora do bundle):** ${diff.extras.map((e) => e.displayName).join(', ')}\n\n`,
            );
          }
          stream.markdown(
            '> 💡 Use o painel visual para mais detalhes e para instalar os pacotes pendentes.\n',
          );
        }
        return;
      }
      ...
      if (cmd === 'explain') {
        const pkg = (await registry.getAll()).find(
          (p: Package) =>
            p.name.includes(prompt.toLowerCase()) ||
            p.displayName.toLowerCase().includes(prompt.toLowerCase()),
        );
        if (!pkg) {
          stream.markdown(
            `❌ Pacote "${prompt}" não encontrado. Tente \`/recommend\` para ver todos os pacotes.`,
          );
          return;
        }
        ...
        if (pkg.agentMeta) {
          ...
          if (pkg.agentMeta.delegatesTo.length > 0) {
            ...
            for (const d of pkg.agentMeta.delegatesTo) {
              stream.markdown(`- ${d}\n`);
            }
          }
          if (pkg.agentMeta.relatedSkills.length > 0) {
            stream.markdown(`\n**Skills Relacionadas:**\n`);
            for (const s of pkg.agentMeta.relatedSkills) {
              stream.markdown(`- ${s}\n`);
            }
          }
        }
      ...
      if (cmd === 'workflow') {
        ...
        for (const pkg of allPkgs) {
          if (pkg.type.value !== 'agent' || !pkg.agentMeta || !installedSet.has(pkg.id)) {
            continue;
          }
          ...
          phaseMap
            .get(phase)!
            .push(`${pkg.agentMeta.category.emoji ?? ''} ${pkg.displayName}`.trim());
        }

        if (phaseMap.size === 0) {
          stream.markdown(
            '> Nenhum agent instalado no workspace atual.\n> Use `/recommend` para descobrir o stack ideal para o seu projeto.\n',
          );
        } else {
      ...
      if (cmd === 'health') {
        ...
        stream.markdown(
          `**Score:** ${report.score}/100 | **Tempo de Scan:** ${report.scanDurationMs}ms\n\n`,
        );
      ...
      if (cmd === 'install') {
        stream.markdown(
          `Para instalar "${prompt}", use a **barra lateral do DescomplicAI** ou execute:\n\n`,
        );
  ...
  if (config.get<boolean>('showWelcome', true) && !hasShownWelcome) {
    ...
    vscode.window
      ...
      .then((choice) => {
        if (choice === 'Abrir Catálogo') {
          vscode.commands.executeCommand('dai-catalog.focus');
        }
      });
  }
}
