...
import { Package, PackageFile, PackageLink, PackageStats } from '../../domain/entities/Package';
...
import {
  CatalogManifestParser,
  CatalogIndex,
  CatalogPackageManifest,
  ManifestLinks,
} from './CatalogManifestParser';
import { UxDiagnosticsService } from '../services/UxDiagnosticsService';
...
export class GitRegistry implements IPackageRepository {
  ...
  private get registryUrl(): string {
    return (
      vscode.workspace.getConfiguration('descomplicai').get<string>('registryUrl') || ''
    ).trim();
  }
  ...
  private async performSync(): Promise<void> {
    ...
    try {
      ...
      this._initialized = true;
      this._logger.info('Catálogo sincronizado.', {
        url,
        packages: this._cache.length,
        bundles: this._bundlesCache.length,
      });
    } catch (error) {
      this._logger.error('Falha ao sincronizar o catálogo.', { url, error });
      UxDiagnosticsService.getInstance().track('service.registrySync.failed', {
        surface: 'background',
      ...
      if (url) {
        void vscode.window.showWarningMessage(
          'Não foi possível sincronizar o catálogo remoto. O catálogo local/customizado continuará disponível.',
        );
      }
    }
  }
  ...
  private async loadFromDisk(rootDir: string): Promise<void> {
    ...
    const index = fs.existsSync(indexPath)
      ? (CatalogManifestParser.parseJsonWithComments(
  ...
  private async loadFromLocalPath(rawPath: string): Promise<void> {
    ...
    const payload = CatalogManifestParser.parseJsonWithComments(
  ...
  private async loadFromCatalogPayload(payload: CatalogIndex, rootDir?: string): Promise<void> {
    ...
    const packageRefs =
      Array.isArray(payload.packages) && payload.packages.length > 0
        ? payload.packages
        : rootDir
          ? this.discoverManifestFiles(rootDir).map((f) =>
              CatalogManifestParser.toRelativePath(rootDir, f),
            )
          : [];
    ...
    for (const pkg of [...packages, ...workspaceCustom]) {
      deduped.set(pkg.id, pkg);
    }

    this._cache = [...deduped.values()];
    this._bundlesCache = rootDir
      ? this.loadBundles(rootDir, payload)
      : this.loadBundlesFromPayload(payload);
  }

  // ─── Hidratação de pacotes ────────────────────────────────────────────────────────────────

  private discoverManifestFiles(rootDir: string): string[] {
    return ['agents', 'skills', 'mcps', 'prompts', 'instructions'].flatMap((folder) =>
      this.collectFiles(path.join(rootDir, folder), (f) => path.basename(f) === 'manifest.json'),
    );
  }

  private loadPackageFromManifestFile(
    ...
    const manifest = CatalogManifestParser.parseJsonWithComments(
      fs.readFileSync(abs, 'utf-8'),
    ) as CatalogPackageManifest;
    return this.packageFromManifestObject(
      manifest,
      rootDir,
      baseRepoUrl,
      index,
      CatalogManifestParser.toRelativePath(rootDir, abs),
    );
  }
  ...
  private packageFromManifestObject(
    ...
    const source =
      ...
      '[inline-manifest]';
    const validated = CatalogManifestParser.validateManifest(
      manifest,
      source,
      (url) => this._fetcher.isSafeRemoteUrl(url),
      this._logger,
    );
    if (!validated) {
      return undefined;
    ...
    const manifestPath =
      manifestPathHint || CatalogManifestParser.asString(manifest.source?.manifestPath);
    const manifestDirRel = manifestPath
    ...
    const name = CatalogManifestParser.slugify(
      CatalogManifestParser.asString(manifest.name) ||
        CatalogManifestParser.asString(manifest.displayName) ||
        CatalogManifestParser.asString(manifest.id) ||
        'package',
    );
    const displayName =
      CatalogManifestParser.asString(manifest.displayName) ||
      CatalogManifestParser.toDisplayName(name);
    ...
    const agentMeta = type.equals(PackageType.Agent)
      ? {
          category: AgentCategory.fromString(
            CatalogManifestParser.asString(manifest.agentMeta?.category) || inferredCategory.value,
          ),
          ...
          workflowPhase:
            CatalogManifestParser.asString(manifest.agentMeta?.workflowPhase) ||
            this.inferWorkflowPhase(inferredCategory, name),
    ...
    return Package.create({
      ...
      installStrategy: {
        kind:
          manifest.install?.strategy === 'mcp-merge' || type.equals(PackageType.MCP)
            ? 'mcp-merge'
            : 'copy',
        targets: installTargets.map((t) => ({
      ...
      ui: {
        longDescription:
          CatalogManifestParser.asString(manifest.ui?.longDescription) ||
          details ||
          readme ||
          description,
  ...
  private resolveFiles(
    ...
    targets: ReturnType<typeof CatalogManifestParser.asInstallTargets>,
    inlineFiles: CatalogPackageManifest['files'],
    type: PackageType,
  ): PackageFile[] {
    if (targets.length > 0 && manifestDirAbs) {
      return targets.map((target) => {
        const srcPath = target.sourcePath
          ? path.join(manifestDirAbs, target.sourcePath)
          : undefined;
        return {
          relativePath: target.targetPath,
          content:
            srcPath && fs.existsSync(srcPath)
    ...
    if (Array.isArray(inlineFiles) && inlineFiles.length > 0) {
      return inlineFiles.flatMap((f) => {
        ...
        return rp
          ? [{ relativePath: rp, content: typeof f.content === 'string' ? f.content : '' }]
          : [];
      });
    }
  ...
  private resolveStats(
    ...
    try {
      const parsed = CatalogManifestParser.parseJsonWithComments(
        fs.readFileSync(statsPath, 'utf-8'),
      ) as Partial<PackageStats>;
      return {
        installsTotal:
          typeof parsed.installsTotal === 'number' ? parsed.installsTotal : def.installsTotal,
        uniqueInstallers:
          typeof parsed.uniqueInstallers === 'number'
            ? parsed.uniqueInstallers
            : def.uniqueInstallers,
        lastInstallAt:
          typeof parsed.lastInstallAt === 'string' ? parsed.lastInstallAt : def.lastInstallAt,
  ...
  private loadBundles(rootDir: string, payload: CatalogIndex): Bundle[] {
    ...
    try {
      const parsed = CatalogManifestParser.parseJsonWithComments(
        fs.readFileSync(bundlesPath, 'utf-8'),
      ) as { bundles?: unknown[] } | unknown[];
  ...
  private loadBundlesFromPayload(payload: CatalogIndex): Bundle[] {
    return (payload.bundles ?? [])
      .map((b) => this.bundleFromManifest(b))
  ...
  private bundleFromManifest(input: unknown): Bundle | undefined {
    ...
    const name = CatalogManifestParser.slugify(
      CatalogManifestParser.asString(b.name) ||
        CatalogManifestParser.asString(b.displayName) ||
        'bundle',
    );
    return Bundle.create({
      ...
      name,
      displayName:
        CatalogManifestParser.asString(b.displayName) || CatalogManifestParser.toDisplayName(name),
  ...
  private async loadWorkspaceCustomPackages(): Promise<Package[]> {
    ...
    return manifests
      .map((m) => this.packageFromManifestObject(m, undefined, '', {}))
  ...
  private serializePackage(pkg: Package): CatalogPackageManifest {
    return {
      ...
      install: {
        ...
        targets: pkg.installStrategy.targets.map((t) => ({
          source: t.sourcePath,
          target: t.targetPath,
          mergeStrategy: t.mergeStrategy,
        })),
      ...
      docs: {
        ...
        links: pkg.docs.links.map((l) => ({ label: l.label, url: l.url })),
      },
      stats: pkg.stats,
  ...
  private inferAgentCategory(name: string, description: string): AgentCategory {
    ...
    if (text.includes('orchestr')) {
      return AgentCategory.Orchestrator;
    }
    if (text.includes('planner') || text.includes('planej')) {
      return AgentCategory.Planner;
    }
    if (
      ...
      return AgentCategory.Guardian;
    }
    if (text.includes('memory') || text.includes('memória') || text.includes('remember')) {
      return AgentCategory.Memory;
    }
    return AgentCategory.Specialist;
  }

  private inferWorkflowPhase(category: AgentCategory, name: string): string {
    if (category.equals(AgentCategory.Orchestrator)) {
      return 'TRIAGE';
    }
    if (category.equals(AgentCategory.Planner)) {
      return 'PLAN';
    }
    if (category.equals(AgentCategory.Memory)) {
      return 'REMEMBER';
    }
    ...
    if (n.includes('review') || n.includes('critic')) {
      return 'CRITIC';
    }
    return 'EXECUTION';
  }
  ...
  private collectFiles(baseDir: string, predicate: (f: string) => boolean): string[] {
    ...
    for (const entry of fs.readdirSync(baseDir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        results.push(...this.collectFiles(full, predicate));
